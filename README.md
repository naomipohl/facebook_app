...
**User** The `users` table should have `name`, `email`, and `password_hash` as columns. I recommend using the scaffold generator for `User`.<br>
...
## Validations
**User** Make sure that the `name` is present and has a minimum length of 2 characters. Make sure that an email is present and unique. Create a custom validation that makes sure the name is capitalized. If it's not, add an error to `name` with the message 'name is not capitalized'.<br>
